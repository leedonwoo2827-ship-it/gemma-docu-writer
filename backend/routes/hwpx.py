...
from doc_mcp.hwpx_vision.lib.md_sections import parse_md_sections, match_to_template_headings, promote_headings_to_top, fix_heading_level_jumps
    ...
    async def stream():
        ...
        try:
            ...
            async for chunk in compose_with_template_headings(filtered, sources):
                ...
                yield f"data: {safe}\n\n"
            # 1) 레벨 점프 보정 (# → ### 처럼 뛰면 ## 로 당김)
            # 2) 가장 얕은 헤딩이 # 가 되도록 자동 승격
