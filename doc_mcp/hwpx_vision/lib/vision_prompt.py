...
3. 레벨별 기호 규칙(절대 다른 기호 쓰지 말 것):
   ...
   - `#####` = L5 세세부, **`·` 사용** (`##### · 세세부`)
   - `######` = L6 최하위, **`※` 사용** (`###### ※ 주석/보충`)
4. **레벨 건너뛰기 금지**: `#` 섹션 바로 아래에 `###` 을 쓰지 말 것. 반드시 `##` 을
   먼저 두어야 함. 마찬가지로 `##` 아래 `####` 금지 — 반드시 `###` 거쳐야.
   즉, 헤딩 레벨은 부모보다 정확히 1단계만 깊어야 함. 레벨 2 이상 점프 금지.
5. 각 `#` 섹션에는 **최소 1개의 `##` 하위 제목 (가./나./다. 등) 이 반드시 있어야 함**.
   내용이 하나뿐이어도 `## 가. 전체 내용` 한 개는 둘 것.
6. 본문은 제공된 원문에 근거하여 작성. 근거 없으면 `[확인 필요]` 명시.
7. 숫자·날짜·기관명은 원문 그대로 인용.
8. Markdown만 출력. 설명·메타코멘트·빈 줄 금지.
9. 표 문법 금지: `| a | b |` 대신 `#### - 항목명: 내용` 목록으로 풀어 쓰기.
10. 공공문서 톤(경어, 객관적 서술).
