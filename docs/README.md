...
원본 HWP/PDF ──(kordoc)──▶ 여러 MD
                ...
                ▼                       ▼
         🎯 HWPX 생성             🎨 PPTX 변환
       (비전 + LLM 본문)          (md2pptx 결정론)
                │                       │
                ▼                       ▼
         결과 HWPX                 결과 PPTX
```

**📝 MD 합성 은 HWPX · PPTX 공통 전처리 단계**. LLM 이 여러 원본 MD 를 하나로 합쳐준다 (현재 UI 에서는 HWPX 탭의 `📝 MD 합성` 버튼으로 트리거). 합성 결과 MD 는 HWPX 생성에 바로 쓰거나, PPTX 탭으로 넘어가서 양식 PPTX 와 함께 변환 입력으로 쓸 수 있다.
