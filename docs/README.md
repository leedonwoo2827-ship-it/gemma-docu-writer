...
```
    ...
    │      결과 PPTX                                   결과 HWPX
    │            │
    └──── (이상 감지 시 analyzer + LLM refiner 로 MD 재작성 제안)
          표 넘침 / 셀 클리핑 / 미매칭 / 양식 shape 수기 삭제 / prose 매핑 실패
```

- **📝 MD 합성 은 HWPX · PPTX 공통 전처리 단계.** LLM 이 여러 원본 MD 를 하나로 합쳐준다 (HWPX 탭의 `📝 MD 합성` 버튼 — 현재 활성 탭 무관). 합성 결과 MD 는 HWPX · PPTX 양쪽 어디에서나 사용 가능.
- **✍ 슬라이드용 글쓰기** 는 PPTX 전용 전처리. 통합 MD 가 줄글 위주면 bullet·표로 재구조화하거나, 양식 PPTX 에 맞게 헤딩을 재편한다.
- **이상 감지 시 피드백 루프**: 결과 PPTX 를 analyzer 가 검사해 표 넘침·셀 클리핑·미매칭 등을 잡아내면, LLM refiner 가 MD 수정안을 제안 → 사용자 승인 → 재변환. 루프는 사람이 제어 (자동 무한루프 X).
